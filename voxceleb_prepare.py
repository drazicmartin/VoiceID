...
def _get_utt_split_lists(
    ...
    for data_folder in data_folders:
        ...
        if split_speaker:
            ...
            for f in glob.glob(path, recursive=True):
                try:
                    spk_id = str(Path(f).as_posix()).split("/wav/")[1].split("/")[0]
                except ValueError:
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True, desc="prepare_csv"):
        # Getting sentence and speaker ids
        try:
            [spk_id, sess_id, utt_id] = Path(wav_file).as_posix().split("/")[-3:]
        except ValueError:
